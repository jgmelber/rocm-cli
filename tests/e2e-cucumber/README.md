...
no XPASS or unexpected failure; the hosted WSL and GPU jobs are non-blocking. The
WSL job validates the real WSL2 host boundary, Linux build, and CLI behavior; it
has no AMD GPU and does not replace GPU-on-WSL hardware coverage. Each workflow
has its own `e2e-report` job: `ci.yml`'s consolidates the GitHub-hosted platforms
(mock and WSL2), and `e2e-selfhosted.yml`'s consolidates the self-hosted ones.
